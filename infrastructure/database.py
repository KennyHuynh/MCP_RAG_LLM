...
from infrastructure.data_util import load_entire_knowledge_base
...
class RAGStorage:
    def __init__(self, db_path="./rag_internal_db", record_db="sqlite:///rag_internal_record_manager.db"):
        ...
        #     self._init_db()
        self._init_db("./infrastructure/rag_data_example")
...
#         print(f"Indexing complete: {indexing_result}")
    ...
    def _init_db(self, directory_path: str):
        initial_docs = load_entire_knowledge_base(directory_path)
        indexing_result = index(
            initial_docs,
            self.record_manager,
            self.vector_store,
